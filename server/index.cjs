...
const authenticateToken = require('../src/middleware/authenticate.cjs'); // Import the authentication middleware
...
//    const {email, filehash} = req.body;
//    const pinataFileId = 
//         async function getFileIdByCID(cid) {
...
//    try {
//       //const user = await User.findOne({email});
//       await pinata.files.public.delete(pinataFileId);
//       return res.status(200).send('File deleted successfully');
//    } catch(err){
//       console.error('Error deleting file:', err.response?.data || err.message || err);
...
app.post('/delete', authenticateToken, async (req, res) => {
  ...
  try {
    // Get the file ID from the CID
    const pinataFileId = await getFileIdByCID(filehash);
    if (!pinataFileId) {
      return res.status(404).send('File not found for given CID');
    }
    ...
    console.error('Error deleting file:', err.response?.data || err.message || err);
    res.status(500).send('Error deleting file - reply from backend');
  }
});
